...
model {
  ...
  skew ~ normal(skew_prior[1], skew_prior[2]);
  // Package up rate coefficients for the ODE solver
  real theta[3] = {beta, gamma, zeta};
  // Set up the estimated time series for disease compartments
  real y_hat[T,3];
  y_hat[1] = y[1];
  y_hat[2:T] = integrate_ode_rk45(sird_dynamics, y[1], ts[1], ts[2:T], theta, x_r, x_i);
  for (t in 1:T) {
    // TODO(MP): y_hat should always be above y
    y[t, 1] ~ skew_normal(y_hat[t, 1], y_hat[t, 1] * sigma_infected, -skew);
...
generated quantities {
  ...
  y_hat[2:T] = integrate_ode_rk45(sird_dynamics, y[1], ts[1], ts[2:T], theta, x_r, x_i);
  for (t in 1:T) {
    log_likelihood[t, 1] = skew_normal_lpdf(y[t, 1] | y_hat[t, 1], y_hat[t, 1] * sigma_infected, -skew);
